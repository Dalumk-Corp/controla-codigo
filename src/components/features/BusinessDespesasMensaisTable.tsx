...
import { BusinessMonthlyExpense, Currency } from '../../types';
import { CURRENCIES, TIPO_DESPESA_MENSAL, RECORRENCIA_OPTIONS } from '../../constants';

interface BusinessDespesasMensaisTableProps {
  expenses: BusinessMonthlyExpense[];
  setExpenses: React.Dispatch<React.SetStateAction<BusinessMonthlyExpense[]>>;
}

const BusinessDespesasMensaisTable: React.FC<BusinessDespesasMensaisTableProps> = ({ expenses, setExpenses }) => {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [currentExpense, setCurrentExpense] = useState<BusinessMonthlyExpense | null>(null);
  ...
  const handleAddRow = () => {
    ...
    const newRow: BusinessMonthlyExpense = {
  ...
  const handleEdit = (expense: BusinessMonthlyExpense) => {
  ...
  const handleEditChange = (field: keyof BusinessMonthlyExpense, value: string | number) => {
    if (currentExpense) {
  ...
  const renderInputField = (expense: BusinessMonthlyExpense, field: keyof BusinessMonthlyExpense) => {
...
export default BusinessDespesasMensaisTable;