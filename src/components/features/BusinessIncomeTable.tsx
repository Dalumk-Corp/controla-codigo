...
import { BusinessIncome, Currency } from '../../types';
import { BUSINESS_INCOME_TYPES, CURRENCIES } from '../../constants';

interface BusinessIncomeTableProps {
  incomes: BusinessIncome[];
  setIncomes: React.Dispatch<React.SetStateAction<BusinessIncome[]>>;
}

type SortField = 'data' | 'descricao' | 'tipo';
type SortOrder = 'asc' | 'desc';

const BusinessIncomeTable: React.FC<BusinessIncomeTableProps> = ({ incomes, setIncomes }) => {
  const [newIncome, setNewIncome] = useState<Omit<BusinessIncome, 'id'>>({
  ...
  const [currentIncome, setCurrentIncome] = useState<BusinessIncome | null>(null);
  ...
  const handleAddIncome = () => {
    ...
    setIncomes([...incomes, { ...newIncome, valor: numericValue, id: crypto.randomUUID() } as BusinessIncome]);
    setNewIncome({
      data: new Date().toISOString().split('T')[0],
      descricao: '',
      tipo: 'Cash',
  ...
  const handleEditChange = (field: keyof BusinessIncome, value: string | number) => {
    if (currentIncome) {
      setCurrentIncome({ ...currentIncome, [field]: value });
    }
  };

  const renderInputField = (income: BusinessIncome, field: keyof BusinessIncome) => {
    ...
    if (currentIncome) {
      switch (field) {
        ...
        case 'tipo':
          return <select value={currentIncome.tipo} onChange={(e) => handleEditChange('tipo', e.target.value as any)} className="bg-slate-700 p-1 rounded w-full">{BUSINESS_INCOME_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select>;
        case 'valor':
            return (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4 mb-6 p-4 border border-slate-700 rounded-lg">
        ...
        <input type="text" placeholder="Serviço/Produto" value={newIncome.descricao} onChange={e => setNewIncome({ ...newIncome, descricao: e.target.value })} className="bg-slate-700 p-2 rounded" />
        <select value={newIncome.tipo} onChange={e => setNewIncome({ ...newIncome, tipo: e.target.value as any })} className="bg-slate-700 p-2 rounded">{BUSINESS_INCOME_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select>
