...
import { Expense, Currency, MonthlyExpense, Debt, Meta } from '../../types';
import { EXPENSE_CATEGORIES, PAYMENT_METHODS, CLASSIFICATIONS, CURRENCIES } from '../../constants';
...
const ExpensesTable: React.FC<ExpensesTableProps> = ({ expenses, setExpenses }) => {
  const [availableCategories, setAvailableCategories] = useState<string[]>(() => {
    const savedCustom = localStorage.getItem('custom_expense_categories');
  ...
  useEffect(() => {
    const customOnly = availableCategories.filter(c => !EXPENSE_CATEGORIES.includes(c));
  ...
  const syncWithMonthlyExpenses = (expense: Expense) => {
    if (expense.tipo === 'Fixo') {
      const savedMonthly = localStorage.getItem('monthlyExpenses');
      const monthlyList: MonthlyExpense[] = savedMonthly ? JSON.parse(savedMonthly) : [];
      ...
      if (!exists) {
        ...
        const newMonthlyEntry: MonthlyExpense = {
        ...
        localStorage.setItem('monthlyExpenses', JSON.stringify([...monthlyList, newMonthlyEntry]));
      }
    }
  };

  const updateLinkedData = (expense: Expense, isAddition: boolean) => {
    const factor = isAddition ? 1 : -1;

    // 1. Check for Debts
    if (expense.categoria === 'Dívida') {
        const savedDebts = localStorage.getItem('debts');
        if (savedDebts) {
            const debts: Debt[] = JSON.parse(savedDebts);
            const debtIndex = debts.findIndex(d => d.credor.trim().toLowerCase() === expense.descricao.trim().toLowerCase());
            if (debtIndex !== -1) {
                const debt = debts[debtIndex];
                debt.parcelasPagas = (debt.parcelasPagas || 0) + factor;
                debt.valorTotal = Math.max(0, debt.valorTotal - (expense.valor * factor));
                if (debt.valorTotal === 0) debt.status = 'Quitado';
                else if (debt.status === 'Quitado') debt.status = 'Regular';
    ...
    // 2. Check for Metas
    if (expense.categoria === 'Meta' || expense.categoria === 'Reserva' || expense.categoria === 'Investimento') {
        const savedMetas = localStorage.getItem('metas');
        if (savedMetas) {
            const metas: Meta[] = JSON.parse(savedMetas);
            const metaIndex = metas.findIndex(m => m.name.trim().toLowerCase() === expense.descricao.trim().toLowerCase());
            if (metaIndex !== -1) {
                const meta = metas[metaIndex];
                meta.saldo = (meta.saldo || 0) + (expense.valor * factor);
  ...
  const handleAddExpense = () => {
    ...
    const updatedExpenses = [...expenses, expenseToAdd as Expense];
    setExpenses(updatedExpenses);

    // Auto-save linked data (Debts, Goals)
    updateLinkedData(expenseToAdd as Expense, true);
    
    // Auto-save to Monthly Expenses if Fixed
    syncWithMonthlyExpenses(expenseToAdd as Expense);
  ...
  const handleDeleteExpense = (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta despesa?')) {
      ...
      if (expenseToDelete) {
        updateLinkedData(expenseToDelete, false);
      }
  ...
  const handleSave = (id: string) => {
    if (currentExpense) {
      ...
      if (oldExpense) {
          updateLinkedData(oldExpense, false);
          updateLinkedData(currentExpense, true);
  ...
  const renderInputField = (expense: Expense, field: keyof Expense) => {
    ...
    if (currentExpense) {
      switch (field) {
        ...
        case 'classificacao':
          return <select value={currentExpense.classificacao} onChange={(e) => handleEditChange('classificacao', e.target.value as any)} className="bg-slate-700 p-1 rounded w-full">{CLASSIFICATIONS.map(c => <option key={c} value={c}>{c}</option>)}</select>;
        case 'tipo':
           return (
            <select value={currentExpense.tipo || 'Variável'} onChange={(e) => handleEditChange('tipo', e.target.value as any)} className="bg-slate-700 p-1 rounded w-full">
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold text-slate-100 mb-4">Adicionar Nova Despesa</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6 p-4 border border-slate-700 rounded-lg">
        ...
        <input type="text" placeholder="Descrição" value={newExpense.descricao} onChange={e => setNewExpense({ ...newExpense, descricao: e.target.value })} className="bg-slate-700 p-2 rounded" />
        ...
        <select value={newExpense.classificacao} onChange={e => setNewExpense({ ...newExpense, classificacao: e.target.value as any })} className="bg-slate-700 p-2 rounded">{CLASSIFICATIONS.map(c => <option key={c} value={c}>{c}</option>)}</select>
        <select value={newExpense.tipo} onChange={e => setNewExpense({ ...newExpense, tipo: e.target.value as any })} className="bg-slate-700 p-2 rounded">
            <option value="Variável">Variável</option>
            <option value="Fixo">Fixo</option>
        </select>
        <input type="text" placeholder="Observação (Opcional)" value={newExpense.observacao} onChange={e => setNewExpense({ ...newExpense, observacao: e.target.value })} className="bg-slate-700 p-2 rounded lg:col-span-1" />
        <button onClick={handleAddExpense} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md transition-colors col-span-full">Adicionar Despesa</button>
      </div>

      <h2 className="text-2xl font-semibold text-slate-100 mb-4">Despesas Registradas</h2>
        ...
        <table className="w-full text-sm text-left text-slate-300">
          <thead className="text-xs text-slate-400 uppercase bg-slate-700">
            <tr>
              ...
              <th className="px-4 py-3 min-w-[140px]">Classificação</th>
          ...
          <tbody>
            {sortedExpenses.map(expense => (
              <tr key={expense.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                ...
                <td className="px-4 py-2 whitespace-nowrap">{renderInputField(expense, 'classificacao')}</td>
